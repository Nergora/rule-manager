...
public class RuleManager : IRuleManager
{
    ...
    public async Task<RuleExecutionResult> ExecuteRuleAsync(string ruleType, object input, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            rule = await provider.GetRuleDefinitionAsync(input, cancellationToken);
        }
        catch (Exception ex)
        {
        ...
        try
        {
            var isValid = await provider.ValidateRuleAsync(rule, input, cancellationToken);
            if (!isValid)
            {
                return new RuleExecutionResult
                {
        ...
        try
        {
            return await _evaluator.EvaluateAsync(rule, input, cancellationToken);
        }
        catch (Exception ex)
        {
